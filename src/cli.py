...
def main() -> int:
    ...
    realesrgan_home = Path(os.environ.get("REALESRGAN_HOME", "/app/realesrgan"))
    ...
    for index, input_path in enumerate(inputs, 1):
        ...
        scale = scale_for_height(info.height, target_height, outscale_override)
    ...
    for index, task in enumerate(tasks, 1):
        ensure_model(model_dir, task.model)
